...
Match = tuple[str, str, OntologyRelation]
...
def _match_tier(
    ...
    *,
    ...
    edges: dict[str, list[tuple[int, str, OntologyRelation]]] = {}
    for prediction in sorted(predictions):
        values: list[tuple[int, str, OntologyRelation]] = []
        for group_index in sorted(available_groups):
            ...
            if choice is not None:
                reference, relation = choice
                values.append((group_index, reference, relation))
        edges[prediction] = values

    matches: dict[int, Match] = {}

    def augment(prediction: str, visited: set[int]) -> bool:
        for group_index, reference, relation in edges[prediction]:
            ...
            if existing is None or augment(existing[0], visited):
                matches[group_index] = (prediction, reference, relation)
                return True
        return False
...
def score_layered_case(
    ...
    *,
    ...
    normalized_predictions = {ontology.normalize(value) for value in predictions}
    groups = [{ontology.normalize(value) for value in group} for group in reference_groups]
    available_predictions = set(normalized_predictions)
    ...
    exact = [
        ...
        for prediction, reference, relation in all_matches.values()
    ...
    counts = Counter(match[2].relation for match in all_matches.values())
    tp = len(all_matches)
    fp = len(normalized_predictions) - tp
    fn = len(groups) - tp
