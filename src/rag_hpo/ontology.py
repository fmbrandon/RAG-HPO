...
def build_entries(
    ...
    *,
    ...
    registry = build_registry(obo_path, addons_path=addons_path, limit=limit)
    return registry_to_artifact_entries(registry)
...
def vectorize(
    *,
    ...
    entries = registry_to_artifact_entries(registry)
    if not entries:
    ...
    return write_artifacts(
        ...
        registry_manifest_sha256=sha256_file(output_dir / REGISTRY_MANIFEST_NAME),
        lexical_sha256=lexical_manifest.lexical_sha256,
        lexical_manifest_sha256=sha256_file(output_dir / LEXICAL_MANIFEST_NAME),
    )
